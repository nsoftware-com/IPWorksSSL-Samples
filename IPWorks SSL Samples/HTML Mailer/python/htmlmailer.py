...
if len(sys.argv) < 4:
  ...
  print("  -u      the username to use for authentication")
  print("  -p      the password to use for authentication")
  ...
  html_mailer.on_ssl_server_authentication = fireSSLServerAuthentication
  ...
  for i in range(0,len(sys.argv)):
    if (sys.argv[i].startswith("-")):
      ...
      if (sys.argv[i]=="-u"):
        html_mailer.set_user(sys.argv[i+1])
      if (sys.argv[i]=="-p"):
        html_mailer.set_password(sys.argv[i+1])

  print("Now sending...")
  try:
    html_mailer.send()

  except IPWorksSSLHtmlmailerError as e:
