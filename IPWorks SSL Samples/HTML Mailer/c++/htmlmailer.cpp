...
int main(int argc, char* argv[])
{
	if (argc < 4) {
		...
		fprintf(stderr, "  -u      the username to use for authentication\n");
		fprintf(stderr, "  -p      the password to use for authentication\n");
		...
		for (int i = 0; i < argc; i++) {
			...
			if (strncmp(argv[i], "-", strlen("-")) == 0) { //check if argument starts with "-"
				...
				if (strcmp(argv[i], "-u") == 0) {
					// args[i+1] corresponds to the value of argument [i]					
					htmlmailer.SetUser(argv[i + 1]);
				}

				if (strcmp(argv[i], "-p") == 0) {
					// args[i+1] corresponds to the value of argument [i]					
					htmlmailer.SetPassword(argv[i + 1]);
				}
			}
		}
