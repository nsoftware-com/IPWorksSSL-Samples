...
class wsserverDemo
{
  ...
  static void Main(string[] args)
  {
    ...
    if (args.Length < 2)
    {
      Console.WriteLine("usage: wsserver cert port");
      Console.WriteLine("  cert      the subject of a certificate in the user's certificate store to be used during SSL/TLS negotiation");
      Console.WriteLine("  port      the TCP port in the local host where the component listens");
      Console.WriteLine("\r\nExample: wsserver certSubject 4444");
    }
    else
    {
      ...
      try
      {
        ...
        wsserver.SSLCert = new Certificate(CertStoreTypes.cstUser, "MY", "", args[args.Length - 2]);
