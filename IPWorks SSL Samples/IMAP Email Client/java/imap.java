...
public class imap extends ConsoleDemo {
	...
	public static void main(String[] args) {
		...
		if (args.length < 4) {
			...
			System.out.println("usage: imap server port username password");
			System.out.println("");
			System.out.println("  server    the name or address of the mail server (IMAP server)");
			System.out.println("  port    the port of the mail server (IMAP server)");
			...
			System.out.println("\r\nExample: imap 127.0.0.1 993 username password");
			...
			try {
				...
				imap1.setMailServer(args[args.length-4]);
				imap1.setMailPort(Integer.parseInt(args[args.length-3]));
				imap1.setUser(args[args.length-2]);
				imap1.setPassword(args[args.length-1]);
				imap1.connect();
				DisplayMenu();
				while (true) {
					lines = 0;
