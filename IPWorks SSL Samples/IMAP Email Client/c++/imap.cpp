...
int main(int argc, char * argv[])
{
	...
	if (argc < 5) {
		...
		fprintf(stderr, "usage: imap server port username password [sslmode]\n");
		fprintf(stderr, "\n");
		fprintf(stderr, "  server    the name or address of the mail server (IMAP server)\n");
		fprintf(stderr, "  port      the port number of the mail server (IMAP server)\n");
		...
		fprintf(stderr, "  sslmode   The SSL mode. Possible values are: auto, explicit, implicit.\n");
		fprintf(stderr, "\nExample (Plaintext):  imap 127.0.0.1 993 username password\n");
		fprintf(stderr, "Example (SSL Auto):     imap 127.0.0.1 993 username password auto\n");
		fprintf(stderr, "Example (SSL Explicit): imap 127.0.0.1 993 username password explicit\n\n");
		printf("Press enter to continue.");
		getchar();
		...
		if (argc > 5)
		{
			...
			else if (!strcmp(argv[5], "explicit"))
			{
		...
		imap.SetMailPort(atoi(argv[2]));
		imap.SetUser(argv[3]);		
		imap.SetPassword(argv[4]);
