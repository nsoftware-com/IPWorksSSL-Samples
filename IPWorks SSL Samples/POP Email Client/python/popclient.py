...
if len(sys.argv) != 7:
  ...
  print("usage: pop_email_client.py popserver port user password mailserver from")
  print("")
  print("  popserver   the name or address of a mail server (internet post office server)")
  print("  port        the port of the mail server")
  ...
  try:
    ...
    pop.set_mail_port(int(sys.argv[2]))
    # pop.set_mail_port(110)
    pop.set_user(sys.argv[3])
    pop.set_password(sys.argv[4])
    pop.connect()

    htmlmailer.set_mail_server(sys.argv[5])
    htmlmailer.set_from(sys.argv[6])
    htmlmailer.set_user(sys.argv[3])
    htmlmailer.set_password(sys.argv[4])
    ...
    while True:
      if str(command).startswith("?"):
        ...
        if mailcount != 0:
          print("\nMsg ".ljust(5) + " " +  "Sender".ljust(50) + " " + "Subject".ljust(50) + "\n")
          for i in range(0,int(mailcount)):
            pop.set_message_number(i+1)
            pop.retrieve()
            print((str(i+1) + ") ").ljust(5) + pop.get_message_from().ljust(50)[:50] + " " + pop.get_message_subject().ljust(50)[:50])
        else:
