...
type
  TFormFtp = class(TForm)
    ...
    procedure ipsFTP1PITrail(Sender: TObject; Direction: Integer;
      const Message: String);
    procedure ipsFTP1EndTransfer(Sender: TObject; Direction: Integer);
    procedure ipsFTP1StartTransfer(Sender: TObject; Direction: Integer);
    procedure ipsFTP1Transfer(Sender: TObject; Direction: Integer;
      BytesTransferred: Int64; PercentDone: Integer; Text: string);
    procedure ipsFTP1DirList(Sender: TObject; const DirEntry, FileName: string;
      IsDir: Boolean; FileSize: Int64; const FileTime: string);
    procedure ipsFTP1SSLServerAuthentication(Sender: TObject;
...
begin
   ...
   if State = OffLineConst then
      ...
      begin
         ...
         begin
            ipsFTP1.ChangeRemotePath('');
            ipsFTP1.RemoteFile := '';
            ipsFTP1.RemoteHost := EditHostName.Text;
            ipsFTP1.User := EditUser.Text;
            ipsFTP1.Password := EditPassword.Text;
            ComboBoxRemHistory.Clear;
            try
               ipsFTP1.Logon;
               State := OnLineConst;
               ComboBoxRemHistory.Items.Insert(0, ipsFTP1.QueryRemotePath());
...
begin
   ...
   begin
      Delete(Name, 1, 7); { Remove first seven characters, '<DIR>  '}
      ipsFTP1.ChangeRemotePath(Name);
      UpdateComboBox(ComboBoxRemHistory, ipsFTP1.QueryRemotePath());
      UpdateRemote;
   end else
...
begin
   ...
   if RadioGroupTransferMode.ItemIndex = 0 then
      ipsFTP1.ChangeTransferMode(Integer(tmASCII))
   else if RadioGroupTransferMode.ItemIndex = 1 then
      ipsFTP1.ChangeTransferMode(Integer(tmBinary))
   else if RadioGroupTransferMode.ItemIndex = 2 then
      ipsFTP1.ChangeTransferMode(Integer(tmDefault));
   except on E: EIPWorksSSL do
      UpdateNotes(E.Message);
   end;
   Screen.Cursor := crDefault;
...
begin
   ...
   begin
      Caption := 'Change Directory';
      LabelWhat.Caption := 'New directory:';
      EditLine.Text := ipsFTP1.QueryRemotePath();

      if ShowModal = mrOk then
      begin
         ipsFTP1.ChangeRemotePath(EditLine.Text);
...
begin
   ...
   begin
      ...
      begin
         try
            ipsFTP1.MakeDirectory(EditLine.Text);
            UpdateRemote;
         except on E: EIPWorksSSL do
            UpdateNotes(E.Message);
         end;
...
begin
   ...
   begin
      ...
      begin
         ...
         ipsFTP1.RemoteFile := NameToRename;
         try
            ipsFTP1.RenameFile(EditLine.Text);
         except on E: EIPWorksSSL do
            UpdateNotes(E.Message);
         end;
         UpdateRemote;
...
begin
   ...
   begin
      ...
      begin
         try
            if IsDir then
               ipsFTP1.RemoveDirectory(NameToDel)
            else
               ipsFTP1.DeleteFile(NameToDel);
         except on E: EIPWorksSSL do
            UpdateNotes(E.Message);
         end;
         UpdateRemote;
...
begin
   ...
   if ListBoxRemFiles.ItemIndex = -1 then
      ...
      Name := ListBoxRemFiles.Items.Strings[ListBoxRemFiles.ItemIndex];
      ipsFTP1.RemoteFile := Name;
      ipsFTP1.LocalFile := Name;
...
begin
   ...
   ipsFTP1.ChangeRemotePath(ComboBoxRemHistory.Text);
   UpdateComboBox(ComboBoxRemHistory, ComboBoxRemHistory.Text);
   UpdateRemote;
end;

procedure TFormFtp.ipsFTP1PITrail(Sender: TObject; Direction: Integer;
  const Message: String);
begin
     UpdateNotes(Message);
end;

procedure TFormFtp.ipsFTP1DirList(Sender: TObject; const DirEntry,
...
procedure TFormFtp.ipsFTP1EndTransfer(Sender: TObject;
  Direction: Integer);
begin
...
procedure TFormFtp.ipsFTP1SSLServerAuthentication(Sender: TObject;
  CertEncoded: string; CertEncodedB: TArray<System.Byte>; const CertSubject, CertIssuer, Status: string;
  var Accept: Boolean);
begin
  Accept:=true;
...
procedure TFormFtp.ipsFTP1StartTransfer(Sender: TObject;
  Direction: Integer);
begin
  FormFtpprogress.ProgressBar1.Position := 0;
end;

procedure TFormFtp.ipsFTP1Transfer(Sender: TObject; Direction: Integer;
  BytesTransferred: Int64; PercentDone: Integer; Text: string);
begin
