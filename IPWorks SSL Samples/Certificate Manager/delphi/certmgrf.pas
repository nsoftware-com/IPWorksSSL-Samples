...
uses
  ...
  generatecsrf, importcsrf, ipscore, ipstypes, ipscertmgr;
...
procedure TFormCertmgr.ipsCertMgr1StoreList(Sender: TObject;
...
begin
  ...
  if pgCertsAndKeys.ActivePage = tabKeys then begin
    ...
    lvKeys.Items.Clear();
    ipsCertMgr1.CertStoreType := cstUser;
...
procedure TFormCertmgr.ipsCertMgr1CertList(Sender: TObject; CertEncoded: string; CertEncodedB: TArray<System.Byte>;
  const CertSubject, CertIssuer, CertSerialNumber: String;
  HasPrivateKey: Boolean);
var
  listView: TListView;
...
begin
  ...
  listItem.SubItems.Add(ipsCertMgr1.CertStore);
...
begin
  ...
  ipsCertMgr1.CertStoreType := cstUser;
  ipsCertMgr1.CertStore := tvUserStores.Selected.Text;
...
begin
  ...
  ipsCertMgr1.CertStoreType := cstMachine;
  ipsCertMgr1.CertStore := tvMachineStores.Selected.Text;
  ipsCertMgr1.ListStoreCertificates;
...
begin
  if lvUserCerts.SelCount > 0 then begin
          ipsCertMgr1.CertStoreType := cstUser;
          ipsCertMgr1.CertStore := lvUserCerts.Selected.SubItems[0];
          ipsCertMgr1.CertSubject := lvUserCerts.Selected.Caption;
          ShowCertInfo;
  end;
end;
...
begin
  if lvMachineCerts.SelCount > 0 then begin
        ipsCertMgr1.CertStoreType := cstMachine;
        ipsCertMgr1.CertStore := lvMachineCerts.Selected.SubItems[0];
        ipsCertMgr1.CertSubject := lvUserCerts.Selected.Caption;
        ShowCertInfo;
        end;
end;
...
begin
  ...
  lvCertInfo.Items.Clear();
	...
	listItem.SubItems.Add(ipsCertMgr1.CertSubject);

	listItem := lvCertInfo.Items.Add;
	listItem.Caption := 'Issuer';
	listItem.SubItems.Add(ipsCertMgr1.CertIssuer);

	listItem := lvCertInfo.Items.Add;
	listItem.Caption := 'Version';
	listItem.SubItems.Add(ipsCertMgr1.CertVersion);

	listItem := lvCertInfo.Items.Add;
	listItem.Caption := 'Serial Number';
	listItem.SubItems.Add(ipsCertMgr1.CertSerialNumber);

	listItem := lvCertInfo.Items.Add;
	listItem.Caption := 'Signature Algorithm';
	listItem.SubItems.Add(ipsCertMgr1.CertSignatureAlgorithm);

	listItem := lvCertInfo.Items.Add;
	listItem.Caption := 'Effective Date';
	listItem.SubItems.Add(ipsCertMgr1.CertEffectiveDate);

	listItem := lvCertInfo.Items.Add;
	listItem.Caption := 'Expiration Date';
	listItem.SubItems.Add(ipsCertMgr1.CertExpirationDate);

	listItem := lvCertInfo.Items.Add;
	listItem.Caption := 'Public Key Algorithm';
	listItem.SubItems.Add(ipsCertMgr1.CertPublicKeyAlgorithm);

	listItem := lvCertInfo.Items.Add;
	listItem.Caption := 'Public Key Length';
	listItem.SubItems.Add(IntToStr(ipsCertMgr1.CertPublicKeyLength));

	listItem := lvCertInfo.Items.Add;
	listItem.Caption := 'Public Key';
	listItem.SubItems.Add(ipsCertMgr1.CertPublicKey);

  if ipsCertMgr1.CertPrivateKeyAvailable then
    spStatus.SimpleText := '* You have a private key that is associated with this certificate.';

end;

procedure TFormCertmgr.ipsCertMgr1KeyList(Sender: TObject;
...
begin
  ...
  ipsCertMgr1.CertStoreType := cstPFXFile;
  lPFXPassword.Caption := InputBox('PFX File Password', 'Please enter the PFX file password:', '');
  ipsCertMgr1.CertStorePassword := lPFXPassword.Caption;
  ipsCertMgr1.CertStore := tPFXFile.Text;
  ipsCertMgr1.ListStoreCertificates;
end;
...
begin
  if lvPFXCerts.SelCount > 0 then begin
        ipsCertMgr1.CertSubject := lvPFXCerts.Selected.Caption;
        ShowCertInfo;
        end;
end;
...
begin
  ShowMessage('IPWorks SSL Certificate Manager');
end;
...
begin
  ...
  if keyName <> '' then ipsCertMgr1.CreateKey(keyName);
...
begin
  ...
  ipsCertMgr1.ExportCertificate(dlgOpen.FileName, password);
...
begin
  ...
  if ipsCertMgr1.CertSubject = '' then begin
...
begin
  ...
  if MessageDlg('Are you sure you want to delete the certificate with subject: ' +
                 ipsCertMgr1.CertSubject + '?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then begin
    ipsCertMgr1.DeleteCertificate;
  end;
...
begin
  ...
  ipsCertMgr1.CertStoreType := cstUser;
  ipsCertMgr1.CertStore := 'MY';
  ipsCertMgr1.ImportCertificate(tPFXFile.Text, lPFXPassword.Caption, lvPFXCerts.Selected.Caption);
  pgCertsAndKeys.ActivePage := tabUserStores;
  Refresh1Click(nil);
